...
from alphamind.utils.helpers import in_debug_mode, is_env_var_true
...
# Initialize Django
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "alphamind.app.settings")
django.setup()
...
# Add CORS middleware
ALPHAMIND_DOMAIN = os.getenv("ALPHAMIND_DOMAIN") or "app.alphamind.dev"
scheme = "https" if not is_env_var_true("ALPHAMIND_NO_HTTPS") else "http"
custom_origins = [f"{scheme}://{ALPHAMIND_DOMAIN.strip()}", f"{scheme}://{ALPHAMIND_DOMAIN.strip()}:*"]
default_origins = [
    ...
    "app://alphamind.dev",  # To allow access from AlphaMind desktop app
]
...
from alphamind.configure import configure_routes, initialize_server, configure_middleware
from alphamind.utils import state
from alphamind.utils.cli import cli
from alphamind.utils.initialization import initialization
from alphamind.database.adapters import ProcessLockAdapters
from alphamind.database.models import ProcessLock
...
def shutdown_scheduler():
    logger.info("🌑 Shutting down AlphaMind")
...
def run(should_start_server=True):
    ...
    logger.info(f"🚒 Initializing AlphaMind v{state.alphamind_version}")
    ...
    logger.info("🌘 Starting AlphaMind")
...
def set_state(args):
    ...
    state.alphamind_version = version("alphamind")


def start_server(app, host=None, port=None, socket=None):
    logger.info("🌖 AlphaMind is ready to engage")
    if socket:
        uvicorn.run(
            app,
            proxy_headers=True,
    ...
    logger.info("🌒 Stopping AlphaMind")

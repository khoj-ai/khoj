...
def completion_with_backoff(
    ...
    groq_max = 4096 if is_groq_api(api_base_url) else MAX_COMPLETION_TOKENS
    model_kwargs["max_completion_tokens"] = model_kwargs.get("max_completion_tokens", groq_max)
    ...
    if is_openai_reasoning_model(model_name, api_base_url):
        ...
        # Groq does not support service_tier — leave model_kwargs clean
        pass

    read_timeout = 300 if is_local_api(api_base_url) else 60
    if os.getenv("ALPHAMIND_LLM_SEED"):
        model_kwargs["seed"] = int(os.getenv("ALPHAMIND_LLM_SEED"))

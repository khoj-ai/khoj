...
from alphamind.database.adapters import ConversationAdapters
...
from alphamind.search_filter.base_filter import BaseFilter
...
from alphamind.utils.rawconfig import FileAttachment
...
def construct_chat_history_for_operator(conversation_history: List[ChatMessageModel], n: int = 6) -> list[AgentMessage]:
    ...
    Only include last n completed turns (i.e with user and alphamind message).
    """
    ...
    for chat in conversation_history:
        ...
        elif chat.by == "alphamind" and chat.message:
...
def message_to_log(
    ...
    default_alphamind_message_metadata = {
        "intent": {"type": "remember", "memory-type": "notes", "query": user_message},
    }
    alphamind_response_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    alphamind_message_metadata = {k: v for k, v in alphamind_message_metadata.items() if v is not None}

    # Create json log from Human's message
    human_log = merge_dicts({"message": user_message, "by": "you"}, user_message_metadata)

    # Create json log from GPT's response
    alphamind_log = merge_dicts(alphamind_message_metadata, default_alphamind_message_metadata)
    alphamind_log = merge_dicts({"message": chat_response, "by": "alphamind", "created": alphamind_response_time}, alphamind_log)
    ...
    try:
        StrictChatMessageModel(**alphamind_log)
    except ValidationError as e:
        logger.error(f"Validation error in alphamind chat message: {e}\nAlphaMind Message: {alphamind_log}\n")

    human_message = ChatMessageModel(**human_log)
    alphamind_message = ChatMessageModel(**alphamind_log)
    chat_history.extend([human_message, alphamind_message])
    return chat_history
...
async def save_to_conversation_log(
    ...
    from alphamind.routers.helpers import ai_update_memories
    ...
    if generated_mermaidjs_diagram:
        alphamind_message_metadata["mermaidjsDiagram"] = generated_mermaidjs_diagram
    ...
    try:
        new_messages = message_to_log(
            ...
            alphamind_message_metadata=alphamind_message_metadata,
            chat_history=[],
        )
    except ValidationError as e:
        new_messages = None
...
def generate_chatml_messages_with_context(
    ...
    for chat in chat_history:
        ...
        if chat.by == "alphamind" and chat.intent and "excalidraw" in chat.intent.type:
        ...
        # Add user query with attached file, images or alphamind response
        if chat.queryFiles:
            raw_query_files = chat.queryFiles
...
def commit_conversation_trace(
    ...
    try:
        ...
        if not repo.config_reader().has_option("user", "email"):
            ...
            repo.config_writer().set_value("user", "email", "promptracer@alphamind.dev").release()

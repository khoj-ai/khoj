...
from alphamind.database.models import ClientApplication, AlphaMindUser, ProcessLock, Subscription
...
from alphamind.utils import constants, state
from alphamind.utils.config import SearchType
from alphamind.utils.helpers import is_none_or_empty

logger = logging.getLogger(__name__)


class AuthenticatedAlphaMindUser(SimpleUser):
...
class UserAuthenticationBackend(AuthenticationBackend):
    def __init__(
        ...
        self.alphaminduser_manager = AlphaMindUser.objects
        self.alphamindapiuser_manager = AlphaMindApiUser.objects
        self._initialize_default_user()
        super().__init__()

    def _initialize_default_user(self):
        if not self.alphaminduser_manager.filter(username="default").exists():
            default_user = self.alphaminduser_manager.create_user(
                username="default",
    ...
    async def authenticate(self, request: HTTPConnection):
        ...
        if current_user and current_user.get("email"):
            try:
                user = (
                    await self.alphaminduser_manager.filter(email=current_user.get("email"))
                ...
                raise HTTPException(
                    status_code=503,
                    detail="Please report this issue on Github, Discord or email team@alphamind.dev and try again later.",
                )
            if user:
                subscribed = await ais_user_subscribed(user)
                if subscribed:
                    return AuthCredentials(["authenticated", "premium"]), AuthenticatedAlphaMindUser(user)
                return AuthCredentials(["authenticated"]), AuthenticatedAlphaMindUser(user)
        ...
        if len(request.headers.get("Authorization", "").split("Bearer ")) == 2:
            ...
            try:
                user_with_token = (
                    await self.alphamindapiuser_manager.filter(token=bearer_token)
                    .select_related("user")
            ...
            if user_with_token:
                ...
                if subscribed:
                    return AuthCredentials(["authenticated", "premium"]), AuthenticatedAlphaMindUser(user_with_token.user)
                return AuthCredentials(["authenticated"]), AuthenticatedAlphaMindUser(user_with_token.user)
        ...
        if client_id:
            ...
            if subscribed:
                return AuthCredentials(["authenticated", "premium"]), AuthenticatedAlphaMindUser(user, client_application)
            return AuthCredentials(["authenticated"]), AuthenticatedAlphaMindUser(user, client_application)

        # No auth required if server in anonymous mode
        if state.anonymous_mode:
            try:
                user = await self.alphaminduser_manager.filter(username="default").prefetch_related("subscription").afirst()
            except (DatabaseError, OperationalError):
...
def initialize_content(user: AlphaMindUser, regenerate: bool, search_type: Optional[SearchType] = None):
...
def configure_routes(app):
    ...
    from alphamind.routers.api import api
    from alphamind.routers.api_agents import api_agents
    from alphamind.routers.api_automation import api_automation
    from alphamind.routers.api_chat import api_chat
    from alphamind.routers.api_content import api_content
    from alphamind.routers.api_memories import api_memories
    from alphamind.routers.api_model import api_model
    from alphamind.routers.notion import notion_router
    from alphamind.routers.web_client import web_client
    ...
    if not state.anonymous_mode:
        from alphamind.routers.auth import auth_router

        app.include_router(auth_router, prefix="/auth")
        logger.info("🔑 Enabled Authentication")

    if state.billing_enabled:
        from alphamind.routers.api_subscription import subscription_router

        app.include_router(subscription_router, prefix="/api/subscription")
        logger.info("💳 Enabled Billing")

    if is_twilio_enabled():
        from alphamind.routers.api_phone import api_phone
...
def configure_middleware(app, ssl_enabled: bool = False):
    ...
    app.add_middleware(SessionMiddleware, secret_key=os.environ.get("ALPHAMIND_DJANGO_SECRET_KEY", "!secret"))

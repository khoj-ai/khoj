...
def fix_malformed_uuids(apps, schema_editor):
    AlphaMindUser = apps.get_model("database", "AlphaMindUser")

    # Track UUID changes for automation cleanup
    uuid_mappings = {}

    # Handle null or empty user UUIDs
    for user in AlphaMindUser.objects.filter(uuid__isnull=True):
    ...
    for user in AlphaMindUser.objects.all():

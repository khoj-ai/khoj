...
from alphamind.configure import initialize_server
from alphamind.database.adapters import UserMemoryAdapters
from alphamind.database.models import (
    Conversation,
    DataStore,
    AlphaMindUser,
)
from alphamind.routers.helpers import extract_facts_from_query
...
class Command(BaseCommand):
    ...
    async def get_users_to_process(self, users_str: Optional[str]) -> List[AlphaMindUser]:
        ...
        if users_str:
            ...
            users = [user async for user in AlphaMindUser.objects.filter(Q(username__in=usernames) | Q(email__in=usernames))]
            return users
        else:
            # Process all users with conversations
            return [user async for user in AlphaMindUser.objects.filter(conversation__isnull=False).distinct()]

    async def get_user_conversations(
        self, user: AlphaMindUser, cutoff_date: Optional[datetime], checkpoint: dict, force: bool
    ) -> List[Conversation]:
    ...
    async def process_conversation_batch(
        self, user: AlphaMindUser, conversations: List[Conversation], apply: bool, checkpoint: dict
    ) -> int:
        ...
        for conversation in conversations:
            try:
                ...
                while i + 1 < len(messages):
                    ...
                    if messages[i].by != "you" or messages[i + 1].by != "alphamind":
                        i += 1
                        continue
    ...
    async def handle_delete_memories(self, usernames: Optional[str], cutoff_date: Optional[datetime], apply: bool):
        ...
        from alphamind.database.models import UserMemory

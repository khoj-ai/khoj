...
from alphamind.utils import constants

if TYPE_CHECKING:
    from sentence_transformers import CrossEncoder, SentenceTransformer

    from alphamind.utils.models import BaseEncoder
...
def get_server_id():
    ...
    Helps count distinct alphamind servers deployed.
    Maintains anonymity by using non-PII random id."""
    # Initialize server_id to None
    server_id = None
    # Expand path to the alphamind env file. It contains persistent internal app data
    ...
    if path.exists(app_env_filename):
        ...
        # Create alphamind config directory if it doesn't exist
...
def log_telemetry(
    ...
    request_body = {
        ...
        "server_version": version("alphamind"),
        "os": platform.system(),
...
command_descriptions = {
    ConversationCommand.General: "Only talk about information that relies on AlphaMind's general knowledge, not your personal knowledge base.",
...
tool_descriptions_for_llm = {
    ...
    ConversationCommand.Online: "To search for the latest, up-to-date information from the internet. Note: **Questions about AlphaMind should always use this data source**",
...
class ImageIntentType(Enum):
    """
    Chat message intent by AlphaMind for image responses.
...
def in_debug_mode():
    """Check if AlphaMind is running in debug mode.
    Set ALPHAMIND_DEBUG environment variable to true to enable debug mode."""
    return is_env_var_true("ALPHAMIND_DEBUG")


def is_promptrace_enabled():
    """Check if AlphaMind is running with prompt tracing enabled.
...
def is_operator_enabled():
    """Check if AlphaMind can operate GUI applications.
    Set ALPHAMIND_OPERATOR_ENABLED env var to true and install playwright to enable it."""
    return is_env_var_true("ALPHAMIND_OPERATOR_ENABLED")


def is_code_sandbox_enabled():
    """Check if AlphaMind can run code in sandbox.
    Set ALPHAMIND_TERRARIUM_URL or E2B api key via env var to enable it."""
    return not is_none_or_empty(os.getenv("ALPHAMIND_TERRARIUM_URL")) or is_e2b_code_sandbox_enabled()
...
def _resolve_api_key(api_key: str, api_base_url: str) -> str:
    """Resolve API key, falling back to env vars for managed providers."""
    if api_base_url and "api.groq.com" in api_base_url:
        return os.environ.get("GROQ_API_KEY") or api_key or ""
    return api_key or ""
...
def get_openai_client(api_key: str, api_base_url: str) -> Union[openai.OpenAI, openai.AzureOpenAI]:
    ...
    resolved_key = _resolve_api_key(api_key, api_base_url)

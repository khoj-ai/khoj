...
from alphamind.processor.conversation.utils import model_to_prompt_size, model_to_tokenizer
from alphamind.utils.constants import (
...
def initialization(interactive: bool = True):
    def _create_admin_user():
        ...
        if not interactive and (not os.getenv("ALPHAMIND_ADMIN_EMAIL") or not os.getenv("ALPHAMIND_ADMIN_PASSWORD")):
            logger.error(
                "🚨 Admin user cannot be created. Please set the ALPHAMIND_ADMIN_EMAIL, ALPHAMIND_ADMIN_PASSWORD environment variables or start server in interactive mode."
            )
            exit(1)
        email_addr = os.getenv("ALPHAMIND_ADMIN_EMAIL") or input("Email: ")
        password = os.getenv("ALPHAMIND_ADMIN_PASSWORD") or input("Password: ")
        admin_user = AlphaMindUser.objects.create_superuser(email=email_addr, username=email_addr, password=password)
    ...
    def _setup_groq_provider(api_key: str):
        """Auto-configure Groq as the managed AI provider using the server-side API key."""
        groq_base_url = "https://api.groq.com/openai/v1"
        groq_models = [
            ("llama-3.3-70b-versatile", "Llama 3.3 70B", 32768),
            ("llama-3.1-8b-instant", "Llama 3.1 8B Instant", 8192),
            ("mixtral-8x7b-32768", "Mixtral 8x7B", 32768),
            ("gemma2-9b-it", "Gemma 2 9B", 8192),
        ]
        try:
            api_entry, _ = AiModelApi.objects.get_or_create(
                name="Groq",
                defaults={"api_key": api_key, "api_base_url": groq_base_url},
            )
            # Always keep the key up-to-date from env
            if api_entry.api_key != api_key or api_entry.api_base_url != groq_base_url:
                api_entry.api_key = api_key
                api_entry.api_base_url = groq_base_url
                api_entry.save()

            for model_name, friendly_name, max_tokens in groq_models:
                ChatModel.objects.get_or_create(
                    name=model_name,
                    defaults={
                        "friendly_name": friendly_name,
                        "model_type": ChatModel.ModelType.OPENAI,
                        "ai_model_api": api_entry,
                        "max_prompt_size": max_tokens,
                        "vision_enabled": False,
                        "price_tier": 0,
                    },
                )
            logger.info("🟢 Groq provider configured with server-managed API key")
        except Exception as e:
            logger.error(f"🚨 Failed to configure Groq provider: {e}", exc_info=True)
    ...
    admin_user = AlphaMindUser.objects.filter(is_staff=True).first()
    ...
    # Auto-configure Groq if GROQ_API_KEY is set and no chat model exists yet
    groq_api_key = os.getenv("GROQ_API_KEY")
    if groq_api_key:
        _setup_groq_provider(groq_api_key)
    ...
    chat_config = ConversationAdapters.get_default_chat_model()
    env_default_chat_model = os.getenv("ALPHAMIND_DEFAULT_CHAT_MODEL")

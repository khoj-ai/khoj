...
from alphamind.utils.helpers import is_env_var_true

# Build paths inside the project like this: BASE_DIR / 'subdir'.
BASE_DIR = Path(__file__).resolve().parent.parent

# SECURITY WARNING: keep the secret key used in production secret!
SECRET_KEY = os.getenv("ALPHAMIND_DJANGO_SECRET_KEY", "!secret")

# Set ALPHAMIND_DOMAIN to custom domain for production deployments.
ALPHAMIND_DOMAIN = os.getenv("ALPHAMIND_DOMAIN") or "alphamind.dev"

# Set ALPHAMIND_ALLOWED_DOMAIN to the i.p or domain of the AlphaMind service on the internal network.
# Useful to set when running the service behind a reverse proxy.
ALPHAMIND_ALLOWED_DOMAIN = os.getenv("ALPHAMIND_ALLOWED_DOMAIN", ALPHAMIND_DOMAIN)
ALLOWED_HOSTS = [f".{ALPHAMIND_ALLOWED_DOMAIN}", "localhost", "127.0.0.1", "[::1]", f"{ALPHAMIND_ALLOWED_DOMAIN}"]

# All Subdomains of ALPHAMIND_DOMAIN are trusted for CSRF
CSRF_TRUSTED_ORIGINS = [
    f"https://*.{ALPHAMIND_DOMAIN}",
    f"https://{ALPHAMIND_DOMAIN}",
    f"http://*.{ALPHAMIND_DOMAIN}",
    f"http://{ALPHAMIND_DOMAIN}",
]

DISABLE_HTTPS = is_env_var_true("ALPHAMIND_NO_HTTPS")

# WARNING: Change this check only if you know what you are doing.
if not os.getenv("ALPHAMIND_DOMAIN"):
    SESSION_COOKIE_DOMAIN = "localhost"
    CSRF_COOKIE_DOMAIN = "localhost"
else:
    # Production Settings
    SESSION_COOKIE_DOMAIN = ALPHAMIND_DOMAIN
    CSRF_COOKIE_DOMAIN = ALPHAMIND_DOMAIN
...
INSTALLED_APPS = [
    ...
    "alphamind.database.apps.DatabaseConfig",
    "unfold",
...
ROOT_URLCONF = "alphamind.app.urls"
...
DB_NAME = os.getenv("POSTGRES_DB", "alphamind")
...
# User Settings
AUTH_USER_MODEL = "database.AlphaMindUser"
...
UNFOLD = {
    "SITE_TITLE": "AlphaMind Admin Panel",
    "SITE_HEADER": "AlphaMind Admin Panel",
    "SITE_URL": "/",
    "SITE_ICON": {
        "light": lambda request: static("assets/icons/alphamind_lantern_128x128.png"),
        "dark": lambda request: static("assets/icons/alphamind_lantern_128x128_dark.png"),
    },
    "SITE_FAVICONS": [
        {
            "rel": "icon",
            "sizes": "32x32",
            "type": "image/svg+xml",
            "href": lambda request: static("assets/icons/alphamind_lantern.svg"),
        },
    ],
}

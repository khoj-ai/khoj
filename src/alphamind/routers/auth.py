...
from alphamind.app.settings import DISABLE_HTTPS, ALPHAMIND_DOMAIN
from alphamind.database.adapters import (
    acreate_alphamind_token,
    aget_or_create_user_by_email,
    aget_user_validated_by_email_verification_code,
    delete_alphamind_token,
    get_alphamind_tokens,
    get_or_create_user,
)
from alphamind.routers.email import send_magic_link_email, send_welcome_email
...
from alphamind.utils.helpers import in_debug_mode
...
def get_configured_base_url() -> str:
    ...
    configured_domain = ALPHAMIND_DOMAIN.strip().rstrip("/")
...
if not state.anonymous_mode:
    ...
    if missing_requirements:
        ...
        error_msg = f"🚨 Start AlphaMind with --anonymous-mode flag or to enable authentication:{requirements_string}"
...
async def generate_token(request: Request, token_name: Optional[str] = None):
    ...
    if token_name:
        token = await acreate_alphamind_token(user=request.user.object, name=token_name)
    else:
        token = await acreate_alphamind_token(user=request.user.object)
    return {
        "token": token.token,
        "name": token.name,
    }
...
def get_tokens(request: Request):
    ...
    tokens = get_alphamind_tokens(user=request.user.object)
    return tokens
...
async def delete_token(request: Request, token: str):
    ...
    return await delete_alphamind_token(user=request.user.object, token=token)
...
async def auth_post(request: Request):
    ...
    alphamind_user = await get_or_create_user(idinfo)

    if alphamind_user:
        request.session["user"] = dict(idinfo)

        if datetime.timedelta(minutes=3) > (datetime.datetime.now(datetime.timezone.utc) - alphamind_user.date_joined):
            asyncio.create_task(send_welcome_email(idinfo["name"], idinfo["email"]))
            update_telemetry_state(
                request=request,
                telemetry_type="api",
                api="create_user__google",
                metadata={"server_id": str(alphamind_user.uuid)},
            )
            logger.log(logging.INFO, f"🥳 New User Created: {alphamind_user.uuid}")

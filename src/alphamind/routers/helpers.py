...
from alphamind.database import adapters
from alphamind.database.adapters import (
    ...
    UserMemoryAdapters,
    aget_user_by_email,
    create_alphamind_token,
    get_default_search_model,
    get_alphamind_tokens,
    get_user_name,
...
from alphamind.processor.content.images.image_to_entries import ImageToEntries
from alphamind.processor.content.markdown.markdown_to_entries import MarkdownToEntries
from alphamind.processor.content.notion.notion_to_entries import NotionToEntries
...
from alphamind.processor.conversation.anthropic.anthropic_chat import (
    anthropic_send_message_to_model,
    converse_anthropic,
)
from alphamind.processor.conversation.google.gemini_chat import (
    converse_gemini,
    gemini_send_message_to_model,
)
from alphamind.processor.conversation.openai.gpt import (
    converse_openai,
    openai_send_message_to_model,
)
from alphamind.processor.conversation.utils import (
    ChatEvent,
    OperatorRun,
    ResearchIteration,
    ResponseWithThought,
...
from alphamind.processor.speech.text_to_speech import is_eleven_labs_enabled
from alphamind.routers.email import is_resend_enabled, send_task_email
from alphamind.routers.twilio import is_twilio_enabled
...
from alphamind.utils.rawconfig import (
    ChatRequestBody,
    FileData,
    LocationData,
    SearchResponse,
)
from alphamind.utils.state import SearchType
from alphamind.utils.yaml import yaml_dump
...
def validate_chat_model(user: AlphaMindUser):
...
async def is_ready_to_chat(user: AlphaMindUser):
    ...
    raise HTTPException(status_code=500, detail="Set your OpenAI API key or enable Local LLM via AlphaMind settings.")
...
def update_telemetry_state(
    ...
    user: AlphaMindUser = request.user.object if request.user.is_authenticated else None
...
async def acreate_title_from_query(query: str, user: AlphaMindUser = None) -> str:
    """
...
async def acheck_if_safe_prompt(system_prompt: str, user: AlphaMindUser = None, lax: bool = False) -> Tuple[bool, str]:
    """
...
def schedule_query(
    q: str, chat_history: List[ChatMessageModel], user: AlphaMindUser, query_images: List[str] = None, tracer: dict = {}
) -> Tuple[str, str, str]:
    """
...
async def search_documents(
    ...
    if not await sync_to_async(EntryAdapters.user_has_entries)(user=user):
        if not agent_has_entries:
            logger.debug("No documents in knowledge base. Use a AlphaMind client to sync and chat with your docs.")
            yield compiled_references, inferred_queries, q
            return
...
class EmailVerificationApiRateLimiter:
    ...
    async def __call__(self, email: str = None):
        ...
        user: AlphaMindUser = await aget_user_by_email(email)
        if not user:
...
class ApiUserRateLimiter:
    ...
    def __call__(self, request: Request):
        ...
        user: AlphaMindUser = request.user.object
        ...
        if not subscribed and count_requests >= self.requests:
            ...
            raise HTTPException(
                status_code=429,
                detail="I'm glad you're enjoying interacting with me! You've unfortunately exceeded your usage limit for today. You can subscribe to increase your usage limit via [your settings](https://app.alphamind.dev/settings) or we can continue our conversation tomorrow?",
            )
    ...
    async def check_websocket(self, websocket: WebSocket):
        ...
        user: AlphaMindUser = websocket.scope["user"].object
        ...
        if not subscribed and count_requests >= self.requests:
            ...
            raise HTTPException(
                status_code=429,
                detail=f"{common_message_prefix} You can subscribe to increase your usage limit via [your settings](https://app.alphamind.dev/settings) or we can continue our conversation {next_window}.",
            )
...
class WebSocketConnectionManager:
    ...
    async def register_connection(self, user: AlphaMindUser, connection_id: str) -> None:
        """Register a new WebSocket connection."""
        await UserRequests.objects.acreate(user=user, slug=f"{self.connection_slug_prefix}{connection_id}")

    async def unregister_connection(self, user: AlphaMindUser, connection_id: str) -> None:
    ...
    async def _cleanup_stale_connections(self, user: AlphaMindUser) -> None:
...
class ConversationCommandRateLimiter:
    ...
    async def update_and_check_if_valid(self, request: Request | WebSocket, conversation_command: ConversationCommand):
        ...
        if not subscribed and count_requests >= self.trial_rate_limit:
            ...
            raise HTTPException(
                status_code=429,
                detail=f"I'm glad you're enjoying interacting with me! You've unfortunately exceeded your `/{conversation_command.value}` command usage limit for today. You can subscribe to increase your usage limit via [your settings](https://app.alphamind.dev/settings) or we can talk about something else for today?",
            )
...
def format_automation_response(scheduling_request: str, executed_query: str, ai_response: str, user: AlphaMindUser) -> bool:
    """
...
def should_notify(original_query: str, executed_query: str, ai_response: str, user: AlphaMindUser) -> bool:
    """
...
def scheduled_chat(
    ...
    url = f"{scheme}://{parsed_url.netloc}/api/chat?client=alphamind"

    # Construct the Headers for the chat API
    headers = {"User-Agent": "AlphaMind", "Content-Type": "application/json"}
    if not state.anonymous_mode:
        # Add authorization request header in non-anonymous mode
        token = get_alphamind_tokens(user)
        if is_none_or_empty(token):
            token = create_alphamind_token(user).token
        else:
            token = token[0].token
...
def get_user_config(user: AlphaMindUser, request: Request, is_detailed: bool = False):
    ...
    if not is_detailed:
        return {
            ...
            "alphamind_version": state.alphamind_version,
        }
    ...
    return {
        ...
        # server settings
        "alphamind_cloud_subscription_url": os.getenv("ALPHAMIND_CLOUD_SUBSCRIPTION_URL"),
...
def get_notion_auth_url(user: AlphaMindUser):

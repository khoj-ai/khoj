...
async def send_magic_link_email(email, unique_id, host):
    ...
    resend.Emails.send(
        {
            "sender": os.environ.get("RESEND_EMAIL", "noreply@alphamind.dev"),
            "to": email,
            "subject": "Your login code to AlphaMind",
            "html": html_content,
        }
    )
...
async def send_welcome_email(name, email):
    ...
    resend.Emails.send(
        {
            "sender": os.environ.get("RESEND_EMAIL", "team@alphamind.dev"),
            "to": email,
            "subject": f"{name}, four ways to use AlphaMind" if name else "Four ways to use AlphaMind",
            "html": html_content,
        }
    )
...
async def send_query_feedback(uquery, kquery, sentiment, user_email):
    if not is_resend_enabled():
        logger.debug(f"Sentiment: {sentiment}, Query: {uquery}, AlphaMind Response: {kquery}")
        return
...
def send_task_email(name, email, query, result, subject, is_image=False):
    ...
    r = resend.Emails.send(
        {
            "sender": f"AlphaMind <{os.environ.get('RESEND_EMAIL', 'alphamind@alphamind.dev')}>",
            "to": email,
            "subject": f"✨ {subject}",
            "html": html_content,
        }

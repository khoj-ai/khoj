...
function pushDataToKhoj(regenerate = false) {
  ...
  for (let i = 0; i < filesDataToPush.length; i += 1000) {
    ...
    let request = axios.post(
      `${hostURL}/api/v1/index/update?force=${regenerate}&client=desktop`,
      formData,
      { headers }
    );
    ...
    .catch((error) => {
      ...
      if (
        ...
        BrowserWindow.getAllWindows().find((win) =>
          win.webContents.getURL().includes("config")
        )
      ) {
        state[
          "error"
        ...
        state[
          "error"
        ] = `Could not connect to Khoj server. Ensure you can connect to it at ${error.address}:${error.port}.`;
      } else {
        currentTime = new Date();
        state[
          "error"
        ] = `Sync was unsuccessful at ${currentTime.toLocaleTimeString()}. Contact team@khoj.dev to report this issue.`;
      }
    })
    .finally(() => {
      // Syncing complete
...
async function handleFileOpen(type) {
  ...
  if (type === "file") {
    ({ canceled, filePaths } = await dialog.showOpenDialog({
      properties: ["openFile"],
      filters: [{ name: "Valid Khoj Files", extensions: validFileTypes }],
    }));
  } else if (type === "folder") {
    ({ canceled, filePaths } = await dialog.showOpenDialog({
...
async function getUserInfo() {
  const getUserInfoURL = `${
    store.get("hostURL") || KHOJ_URL
  }/api/v1/user?client=desktop`;
  const headers = { Authorization: `Bearer ${store.get("khojToken")}` };
  try {

;;; alphamind-tests.el --- Test suite for alphamind.el  -*- lexical-binding: t -*-
...
;; This file contains the test suite for alphamind.el.
...
(ert-deftest alphamind-tests--extract-entries-as-markdown ()
  "Test `json-response', `query' from API formatted as markdown."
  (let ((user-query "Become God")
        (json-response-from-alphamind-backend
...
")))
    (should
     (equal
      (alphamind--extract-entries-as-markdown json-response-from-alphamind-backend user-query nil)
      "\
# Become God\n\
## Upgrade\n\
\n\
...
(ert-deftest alphamind-tests--extract-entries-as-org ()
...
")))
    (should
     (equal
      (alphamind--extract-entries-as-org json-response-from-alphamind-backend user-query nil)
      "\
* Become God\n\
** Upgrade\n\
\n\
...
(ert-deftest alphamind-tests--get-current-outline-entry-text ()
...
(ert-deftest alphamind-tests--get-current-paragraph-text ()
...
(ert-deftest alphamind-tests--render-files-to-add-request-body ()
  ...
  (let ((upgrade-file (make-temp-file "upgrade" nil ".org" "# Become God\n## Upgrade\n\nPenance to Immortality\n\n"))
    ...
    (unwind-protect
        (progn
          (should
           (equal
            (alphamind--render-update-files-as-request-body (list upgrade-file act-file) "alphamind")
            (format
             "\n--alphamind\r\n\
...
(ert-deftest alphamind-tests--render-files-to-add-delete-in-request-body ()
  "Test files are formatted into a multi-part http request body"
  (let ((upgrade-file (make-temp-file "upgrade" nil ".org" "# Become God\n## Upgrade\n\nPenance to Immortality\n\n"))
        (act-file (make-temp-file "act" nil ".org" "## Act\n\nRule everything\n\n")))
    (unwind-protect
        (progn
          (should
           (equal
            (alphamind--render-delete-files-as-request-body (list upgrade-file act-file "/tmp/deleted-file.org") "alphamind")
            (format
             "\n--alphamind\r\n\
Content-Disposition: form-data; name=\"files\"; filename=\"%s\"\r\n\
Content-Type: text/org\r\n\r\n\
...
(ert-deftest alphamind-tests--extract-online-references ()
  (let* (;; Arrange
         ...
         ;; Act
         (result (alphamind--extract-online-references
...
(provide 'alphamind-tests)

;;; alphamind-tests.el ends here

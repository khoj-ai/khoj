...
export const ChatInputArea = forwardRef<HTMLTextAreaElement, ChatInputProps>((props, ref) => {
    ...
    const [showCommandList, setShowCommandList] = useState(false);
        const [showFileFilter, setShowFileFilter] = useState(false);
            const [fileFilterSuggestions, setFileFilterSuggestions] = useState<string[]>([]);
                                    ...
                                    const response = await fetch("/api/content/files");
                                                if (!response.ok) throw new Error("Failed to fetch files");
                                                            const data = await response.json();
                                                                        return data.files.map((file: any) => file.file_name);
                                                                                } catch (error) {
    ...
    useEffect(() => {
        ...
        if (message.startsWith("/") && message.split(" ").length === 1) {
            ...
            setShowCommandList(false);
                    ...
                    // Detect file: filter pattern
                            const fileFilterMatch = message.match(/file:(\S*)$/);
                                    if (fileFilterMatch) {
                                                    const query = fileFilterMatch[1];
                                                                setFileFilterQuery(query);
                                                                            if (!showFileFilter) {
                                                                                                fetchUserFiles().then(files => {
                                                                                                                        setFileFilterSuggestions(files);
                                                                                                                                            setShowFileFilter(true);
                                                                                                                                                            });
                                                                                                                                                                        } else {
                                                                                                                                                                                            // Filter existing suggestions
                                                                                                                                                                                                            fetchUserFiles().then(allFiles => {
                                                                                                                                                                                                                                    const filtered = allFiles.filter(f => f.toLowerCase().includes(query.toLowerCase()));
                                                                                                                                                                                                                                                        setFileFilterSuggestions(filtered);
                                                                                                                                                                                                                                                                        });
                                                                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                                                                            } else {
            ...
            {showCommandList && (
                <div className="flex justify-center text-center">
                    <Popover open={showCommandList} onOpenChange={setShowCommandList}>
                        ...
                        {showFileFilter && (
                                            <Popover open={showFileFilter} onOpenChange={setShowFileFilter}>
                                                                <PopoverTrigger className="flex justify-center text-center"></PopoverTrigger>
                                                                                    <PopoverContent
                                                                                                            onOpenAutoFocus={(e) => e.preventDefault()}
                                                                                                                                    className={`${props.isMobileWidth ? "w-[100vw]" : "w-full"} rounded-md`}
                                                                                                                                                            side="bottom"
                                                                                                                                                                                    align="center"
                                                                                                                                                                                                            sideOffset={props.conversationId ? 0 : 80}
                                                                                                                                                                                                                                >
                                                                                                                                                                                                                                                        <Command className="max-w-full">
                                                                                                                                                                                                                                                                                    <CommandInput placeholder="Filter files..." value={fileFilterQuery} className="hidden" />
                                                                                                                                                                                                                                                                                                                <CommandList>
                                                                                                                                                                                                                                                                                                                                                <CommandEmpty>No files found.</CommandEmpty>
                                                                                                                                                                                                                                                                                                                                                                                <CommandGroup heading="Files">
                                                                                                                                                                                                                                                                                                                                                                                                                    {fileFilterSuggestions.map((filename) => (
                                                                                                                                                                                                                                                                                                                                                                                                                                                                <CommandItem
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            key={filename}
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        onSelect={() => {
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            const newMessage = message.replace(/file:\S*$/, `file:${filename} `);

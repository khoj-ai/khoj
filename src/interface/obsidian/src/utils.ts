...
export async function updateContentIndex(vault: Vault, setting: KhojSetting, lastSync: Map<TFile, number>, regenerate: boolean = false, userTriggered: boolean = false): Promise<Map<TFile, number>> {
    ...
    console.log(`🔄 Khoj: Starting content synchronization...`)
    ...
    console.log(`📊 Khoj: Found ${files.length} eligible files for synchronization`);

    let countOfFilesToIndex = 0;
    let countOfFilesToDelete = 0;
    let successfullyIndexedFiles = 0;
    ...
    const MAX_FILES_PER_BATCH = 1000; // Server limit
    let currentBatch = [];

    console.log(`🔍 Khoj: Analyzing files to synchronize...`);
    ...
    for (const file of files) {
        ...
        try {
            ...
            if (currentBatch.length > 0 && (currentBatchSize + fileSize > MAX_BATCH_SIZE || currentBatch.length >= MAX_FILES_PER_BATCH)) {
            ...
            console.error(`❌ Khoj: Error reading file ${file.path}:`, error);
            failedFiles.push(file.path);
        }
    }

    // Add any previously synced files to be deleted to batches
    ...
    for (const lastSyncedFile of lastSync.keys()) {
        if (!files.includes(lastSyncedFile)) {
            ...
            // Check if adding this deletion would exceed file count limit
    ...
    // Add final batch if not empty, but split if it exceeds limits
    while (currentBatch.length > 0) {
        if (currentBatch.length <= MAX_FILES_PER_BATCH) {
            ...
            const splitBatch = currentBatch.splice(0, MAX_FILES_PER_BATCH);
            fileData.push(splitBatch);
        }
    }

    console.log(`📦 Khoj: ${countOfFilesToIndex} files to index, ${countOfFilesToDelete} files to delete`);
    console.log(`📦 Khoj: ${fileData.length} data batches prepared for upload`);

    // Log batch sizes for debugging
    fileData.forEach((batch, index) => {
        console.log(`📋 Khoj: Batch ${index + 1}: ${batch.length} files (max ${MAX_FILES_PER_BATCH})`);
    });
    ...
    if (regenerate) {
        console.log(`🗑️ Khoj: Clearing existing content (regeneration)...`);
    ...
    for (const contentType of contentTypesToDelete) {
        console.log(`🗑️ Khoj: Deleting content type: ${contentType}`);
        try {
            ...
            if (!response.ok) {
                ...
                console.error(`❌ Khoj: Error deleting content type ${contentType}: ${response.status} - ${errorText}`);
                error_message = "❗️Failed to clear existing content index";
                fileData = [];
                break;
            } else {
                console.log(`✅ Khoj: Content type ${contentType} deleted successfully`);
            }
        } catch (error) {
            console.error(`❌ Khoj: Network error while deleting content type ${contentType}:`, error);
            error_message = "❗️Connection error while clearing index";
            fileData = [];
            break;
        }
    }
    ...
    for (const batch of fileData) {
        batchIndex++;
        const progressBar = createProgressBar(batchIndex - 1, fileData.length);
        console.log(`📤 Khoj: Uploading batch ${batchIndex}/${fileData.length} (${batch.length} files, max ${MAX_FILES_PER_BATCH}) ${progressBar}`);

        // Validate batch size before processing
        if (batch.length > MAX_FILES_PER_BATCH) {
            console.error(`❌ Khoj: Batch ${batchIndex} has ${batch.length} files, exceeding limit of ${MAX_FILES_PER_BATCH}`);
            error_message = `❗️Batch size error: ${batch.length} files exceeds server limit of ${MAX_FILES_PER_BATCH}`;
            break;
        }
        ...
        while (retryCount < maxRetries && !success) {
            try {
                console.log(`🔄 Khoj: Attempt ${retryCount + 1}/${maxRetries} for batch ${batchIndex}`);
                ...
                if (!response.ok) {
                    const errorText = await response.text();
                    console.error(`❌ Khoj: HTTP error ${response.status} for batch ${batchIndex}: ${errorText}`);

                    if (response.status === 429) {
                        if (errorText.includes("Too much data")) {
                        ...
                        error_message = `❗️Access denied (403). Check your Khoj API key in settings.`;
                        console.error(`❌ Khoj: Authentication error - Invalid or expired API key`);
                        break; // Don't retry on 403
                    } else if (response.status === 400) {
                        error_message = `❗️Invalid request (400). Incorrect data format.`;
                        console.error(`❌ Khoj: Request format error - ${errorText}`);
                        break; // Don't retry on 400
                    } else if (response.status === 404) {
                        error_message = `❗️Could not connect to Khoj server. Check server URL.`;
                    ...
                    successfullyIndexedFiles += batch.length;
                    totalProcessedFiles += batch.length;
                    const finalProgressBar = createProgressBar(batchIndex, fileData.length);
                    console.log(`✅ Khoj: Batch ${batchIndex}/${fileData.length} synced successfully (${batch.length} files) ${finalProgressBar}`);
                    success = true;
                }
            } catch (error) {
                console.error(`❌ Khoj: Network error for batch ${batchIndex}, attempt ${retryCount + 1}:`, error);
                if (retryCount === maxRetries - 1) {
                    error_message = `❗️Network connection error. Check your internet connection and Khoj server URL.`;
                    batch.forEach(fileItem => failedFiles.push(fileItem.path));
                }
            }
            ...
            if (!success && retryCount < maxRetries) {
                ...
                console.log(`⏳ Khoj: Waiting ${waitTime}ms before retry...`);
                await new Promise(resolve => setTimeout(resolve, waitTime));
            }
        }

        if (error_message) {
            break; // Stop processing if we have a critical error
        }
    }

    // Update last sync time for each successfully indexed file
    const successfulFiles = files.filter(file =>
        responses.some(response => response.includes(file.path)) &&
        !failedFiles.includes(file.path)
    );

    successfulFiles.forEach(file => {
        lastSync.set(file, new Date().getTime());
    });

    // Remove files that were deleted from last sync
    const successfullyDeletedFiles = filesToDelete.filter(file =>
        responses.some(response => response.includes(file.path))
    );
    successfullyDeletedFiles.forEach(file => lastSync.delete(file));

    // Final status reporting
    const totalFilesProcessed = successfulFiles.length + successfullyDeletedFiles.length;
    const totalFilesAttempted = countOfFilesToIndex + countOfFilesToDelete;

    console.log(`📊 Khoj: Synchronization completed - ${totalFilesProcessed}/${totalFilesAttempted} files processed successfully`);

    if (failedFiles.length > 0) {
        console.warn(`⚠️ Khoj: ${failedFiles.length} files failed:`, failedFiles);
    }

    if (error_message) {
        new Notice(error_message);
        console.error(`❌ Khoj: Synchronization failed with error`);
    } else {
        const successMessage = `✅ Khoj index updated. Synced: ${successfulFiles.length} files, Deleted: ${successfullyDeletedFiles.length} files.`;
        if (userTriggered) new Notice(successMessage);
        console.log(`✅ Khoj: ${successMessage}`);

        // Verify all files are synced if this was a user-triggered sync
        if (userTriggered && totalFilesProcessed < totalFilesAttempted) {
            const warningMessage = `⚠️ Warning: ${totalFilesAttempted - totalFilesProcessed} files could not be synchronized. Check console logs for details.`;
            new Notice(warningMessage);
            console.warn(`⚠️ Khoj: ${warningMessage}`);
        }
    }

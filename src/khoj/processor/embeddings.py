...
class EmbeddingsModel:
    def __init__(self, model_name: str = "thenlper/gte-small", huggingface_inference_endpoint: str = None):
        self.encode_kwargs = {"normalize_embeddings": True}
        self.model_kwargs = {"device": get_device()}
        self.model_name = model_name
        self.huggingface_inference_endpoint = huggingface_inference_endpoint
        self.embeddings_model = SentenceTransformer(self.model_name, **self.model_kwargs)
        self.huggingface_api_key = os.getenv("HUGGINGFACE_API_KEY")

    def embed_query(self, query):
        if self.huggingface_api_key is not None and self.huggingface_inference_endpoint is not None:
            target_url = f"{self.huggingface_inference_endpoint}"

...
By default, most of the instructions for self-hosting AlphaMind assume a single user, and so the default configuration is to run in anonymous mode. However, if you want to enable authentication, you can do so either with with [Magic Links](#using-magic-links) or [Google OAuth](#using-google-oauth) as shown below. This can be helpful to make AlphaMind securely accessible to you and your team.

:::tip[Note]
Remove the `--anonymous-mode` flag from your alphamind start up command or docker-compose file to enable authentication.
:::
...
For this method, you'll need to use the prod version of the AlphaMind package. You can install it as below:

<Tabs groupId="server" queryString>
  <TabItem value="docker" label="Docker">
  Update your `docker-compose.yml` to use the prod image
      ```bash
      image: ghcr.io/alphamind-ai/alphamind-cloud:latest
      ```
  </TabItem>

  <TabItem value="pip" label="Pip">
  ```bash
  pip install alphamind[prod]
  ```
  </TabItem>
</Tabs>

To set up your self-hosted AlphaMind with Google Auth, you need to create a project in the Google Cloud Console and enable the Google Auth API.
...
![Google auth login project settings](https://github.com/alphamind-ai/alphamind/assets/65192171/9bcbf6f4-197d-4d0c-973a-c10b1331c892)
...
That's it! That should be all you have to do. Now, when you reload AlphaMind without `--anonymous-mode`, you should be able to use your Google account to sign in.

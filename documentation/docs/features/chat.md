...
You can configure AlphaMind to chat with you about anything. When relevant, it'll use any notes or documents you shared with it to respond. It acts as an excellent research assistant, search engine, or personal tutor.

<img src="https://assets.alphamind.dev/vision_chat_example.png" alt="Chat on Web" style={{width: '400px'}}/>
...
### Use
1. Open AlphaMind Chat
    - **On Web**: Open [/chat](https://app.alphamind.dev/chat) in your web browser
    - **On Obsidian**: Search for *AlphaMind: Chat* in the [Command Palette](https://help.obsidian.md/Plugins/Command+palette)
    - **On Emacs**: Run `M-x alphamind <user-query>`
2. Enter your queries to chat with AlphaMind. Use [slash commands](#commands) and [query filters](/miscellaneous/query-filters) to change what AlphaMind uses to respond


#### Details
1. Your query is used to retrieve the most relevant notes, if any, using AlphaMind search using RAG.
...
#### Commands
Slash commands allows you to change what AlphaMind uses to respond to your query
- **/notes**: Limit chat to only respond using your notes, not just AlphaMind's general world knowledge as reference
- **/general**: Limit chat to only respond using AlphaMind's general world knowledge, not using your notes as reference
...
- **/help**: Use /help to get all available commands and general information about AlphaMind

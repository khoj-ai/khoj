...
- To use fastCRW, a Firecrawl-compatible web scraper in a single binary, set the `CRW_API_KEY` environment variable to your [fastCRW](https://fastcrw.com) API key. Self-host the engine and set `CRW_API_URL` to point Khoj at your own server.
...
- For a Firecrawl-compatible webpage reader in a single binary, you can use [fastCRW](https://fastcrw.com/). Create a new [Webscraper](http://localhost:42110/server/admin/database/webscraper/add/). Set your fastCRW API key to the Api Key field, and set the type to Crw. Set the Api Url field to point at your own server when self-hosting.

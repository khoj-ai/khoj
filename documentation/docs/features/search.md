...
### Use
1. Open AlphaMind Search
  - **On Web**: Open https://app.alphamind.dev/ in your web browser
  - **On Obsidian**: Click the *AlphaMind search* icon 🔎 on the [Ribbon](https://help.obsidian.md/User+interface/Workspace/Ribbon) or Search for *AlphaMind: Search* in the [Command Palette](https://help.obsidian.md/Plugins/Command+palette)
...
1. When you sync you documents with AlphaMind, it uses the bi-encoder model to create and store meaning vectors of (chunks of) your documents
...
### Setup (Self-Hosting)
You are **not required** to configure the search model config when self-hosting. AlphaMind sets up decent default local search model config for general use.
...
1. Open the [SearchModelConfig](http://localhost:42110/server/admin/database/searchmodelconfig/) page on your AlphaMind admin panel.
...
7. Save the search model configs and restart your AlphaMind server to start using your new, updated search config.
...
[^1]: AlphaMind uses the first search model config named `default` it finds on startup as the search model config for that session

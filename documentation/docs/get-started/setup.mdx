...
# Self-Host
Learn about how to self-host AlphaMind on your own machine.

Benefits to self-hosting:
1. **Privacy**: Your data will never have to leave your private network. You can even use AlphaMind without an internet connection if deployed on your personal computer.
2. **Customization**: You can customize AlphaMind to your liking, from models, to host URL, to feature enablement.
...
## Setup AlphaMind
These are the general setup instructions for self-hosted AlphaMind.
You can install the AlphaMind server using either [Docker](?server=docker) or [Pip](?server=pip).

:::info[First Run]
Restart your AlphaMind server after the first run to ensure all settings are applied correctly.
:::
    ...
    <Tabs groupId="operating-systems" queryString="os">
      <TabItem value="macos" label="MacOS">
        ...
        - *Option 2*: Use [Homebrew](https://brew.sh/) to install Docker and Docker Compose.
          ...
          ```
        <h3>Setup</h3>
        1. Download the AlphaMind docker-compose.yml file [from Github](https://github.com/alphamind-ai/alphamind/blob/master/docker-compose.yml)
           ```shell
        ...
        2. Configure the environment variables in the `docker-compose.yml`
           - Set `ALPHAMIND_ADMIN_PASSWORD`, `ALPHAMIND_DJANGO_SECRET_KEY` (and optionally the `ALPHAMIND_ADMIN_EMAIL`) to something secure. This allows you to customize AlphaMind later via the admin panel.
        ...
        3. Start AlphaMind by running the following command in the same directory as your docker-compose.yml file.
           ```shell
           cd ~/.alphamind
           docker-compose up
           ```
...
:::info[Remote Access]
By default AlphaMind is only accessible on the machine it is running. To access AlphaMind from a remote machine see [Remote Access Docs](/advanced/remote).
:::

Your setup is complete once you see `🌖 AlphaMind is ready to engage` in the server logs on your terminal.
  </TabItem>
  <TabItem value="pip" label="Pip">

<h3>1. Install AlphaMind Server</h3>
...
Run the following command in your terminal to install the AlphaMind server.
  ...
  <Tabs groupId="operating-systems" queryString="os">
    ...
    ```shell
CMAKE_ARGS="-DGGML_METAL=on" python -m pip install 'alphamind[local]'
    ```
        </TabItem>
        <TabItem value="intel" label="Intel">
    ```shell
python -m pip install 'alphamind[local]'
    ```
        </TabItem>
      </Tabs>
    </TabItem>
    <TabItem value="windows" label="Windows">
      ...
      <Tabs groupId="gpu" queryString="gpu">
       ...
       ```shell
 # Install AlphaMind
 py -m pip install 'alphamind[local]'
      ```
        </TabItem>
 ...
 py -m pip install 'alphamind[local]'
    ...
    <TabItem value="linux" label="Linux">
      <Tabs groupId="gpu" queryString="gpu">
        ...
        <TabItem value="nvidia" label="NVIDIA (CUDA) GPU">
          ```shell
          CMAKE_ARGS="-DGGML_CUDA=on" FORCE_CMAKE=1 python -m pip install 'alphamind[local]'
          ```
        </TabItem>
        <TabItem value="amd" label="AMD (ROCm) GPU">
          ```shell
          CMAKE_ARGS="-DGGML_HIPBLAS=on" FORCE_CMAKE=1 python -m pip install 'alphamind[local]'
          ```
        </TabItem>
        <TabItem value="vulkan" label="VULKAN GPU">
          ```shell
          CMAKE_ARGS="-DGGML_VULKAN=on" FORCE_CMAKE=1 python -m pip install 'alphamind[local]'
          ```
        </TabItem>
      </Tabs>
    </TabItem>
...
<h3>2. Start AlphaMind Server</h3>

Run the following command from your terminal to start the AlphaMind service.

```shell
USE_EMBEDDED_DB="true" alphamind --anonymous-mode
```
`--anonymous-mode` allows access to AlphaMind without requiring login. This is usually fine for local only, single user setups. If you need authentication follow the [authentication setup docs](/advanced/authentication).
...
3. Your setup is complete once you see `🌖 AlphaMind is ready to engage` in the server logs on your terminal!


:::tip[Auto Start]
To start AlphaMind automatically in the background use [Task scheduler](https://www.windowscentral.com/how-create-automated-task-using-task-scheduler-windows-10) on Windows or [Cron](https://en.wikipedia.org/wiki/Cron) on Mac, Linux (e.g. with `@reboot alphamind`)
:::
  </TabItem>
</Tabs>
...
### Add Chat Models
<h4>Login to the AlphaMind Admin Panel</h4>
...
If using a load balancer/reverse_proxy in front of your AlphaMind server: Set the environment variable ALPHAMIND_ALLOWED_DOMAIN=your-internal-ip-or-domain to avoid this error.
If unset, it defaults to ALPHAMIND_DOMAIN.
:::

:::info[DISALLOWED HOST or Bad Request (400) Error]
You may hit this if you try access AlphaMind exposed on a custom domain (e.g. 192.168.12.3 or example.com) or over HTTP.
Set the environment variables ALPHAMIND_DOMAIN=your-external-ip-or-domain and ALPHAMIND_NO_HTTPS=True if required to avoid this error.
:::
...
Setup which chat model you'd want to use. AlphaMind supports local and online chat models.
...
3. Restart the AlphaMind server to load models available on your local ai model provider
...
AlphaMind uses these chat model for all intermediate steps like intent detection, web search etc.
:::
...
- You can chat with your notes and documents using AlphaMind.
- AlphaMind can keep your files and folders synced using the AlphaMind [Desktop](/clients/desktop#setup), [Obsidian](/clients/obsidian#setup) or [Emacs](/clients/emacs#setup) clients.
...
### Setup AlphaMind Clients
The AlphaMind web app is available by default to chat, search and configure AlphaMind.<br />
You can also install a AlphaMind client to easily access it from Obsidian, Emacs, Whatsapp or your OS and keep your documents synced with AlphaMind.

:::info[Note]
Set the host URL on your clients settings page to your AlphaMind server URL. By default, use `http://127.0.0.1:42110` or `http://localhost:42110`. Note that `localhost` may not work in all cases.
:::

<Tabs groupId="client" queryString>
  <TabItem value="desktop" label="Desktop">
  - Read the AlphaMind Desktop app [setup docs](/clients/desktop#setup).
  </TabItem>
  <TabItem value="emacs" label="Emacs">
  - Read the AlphaMind Emacs package [setup docs](/clients/emacs#setup).
  </TabItem>
  <TabItem value="obsidian" label="Obsidian">
  - Read the AlphaMind Obsidian plugin [setup docs](/clients/obsidian#setup).
  </TabItem>
  <TabItem value="whatsapp" label="Whatsapp">
  - Read the AlphaMind Whatsapp app [setup docs](/clients/whatsapp).
  </TabItem>
</Tabs>
...
### Upgrade Server
  <Tabs groupId="server" queryString>
    <TabItem value="pip" label="Pip">
      ```shell
      pip install --upgrade alphamind
      ```
      *Note: To upgrade to the latest pre-release version of the alphamind server run below command*
    </TabItem>
    <TabItem value="docker" label="Docker">
      ...
      ```shell
      # Windows users should use their WSL2 terminal to run these commands
      cd ~/.alphamind  # assuming your alphamind docker-compose.yml file is here
...
### Upgrade Clients
  <Tabs groupId="client" queryString>
    <TabItem value="desktop" label="Desktop">
      ...
      - You can manually download the latest version from the [AlphaMind Website](https://alphamind.dev/downloads).
    </TabItem>
    <TabItem value="emacs" label="Emacs">
    - Use your Emacs Package Manager to Upgrade
    - See [alphamind.el package setup](/clients/emacs#setup) for details
    </TabItem>
    <TabItem value="obsidian" label="Obsidian">
      - Upgrade via the Community plugins tab on the settings pane in the Obsidian app
      - See the [alphamind plugin setup](/clients/obsidian#setup) for details
    </TabItem>
  </Tabs>
...
### Uninstall Server
  <Tabs groupId="server" queryString>
    <TabItem value="pip" label="Pip">
    ```shell
    # uninstall alphamind server
    pip uninstall alphamind

    # delete alphamind postgres db
    dropdb alphamind -U postgres
    ```
    </TabItem>
    <TabItem value="docker" label="Docker">
...
### Uninstall Clients
  <Tabs groupId="client" queryString>
    <TabItem value="desktop" label="Desktop">
    Uninstall the AlphaMind Desktop client in the standard way from your OS.
    </TabItem>
    <TabItem value="emacs" label="Emacs">
    Uninstall the AlphaMind Emacs package in the standard way from Emacs.
    </TabItem>
    <TabItem value="obsidian" label="Obsidian">
...
#### Dependency conflict when trying to install AlphaMind python package with pip
- **Reason**: When conflicting dependency versions are required by AlphaMind vs other python packages installed on your system
- **Fix**: Install AlphaMind in a python virtual environment using [venv](https://docs.python.org/3/library/venv.html) or [pipx](https://pypa.github.io/pipx) to avoid this dependency conflicts
- **Process**:
  1. Install [pipx](https://pypa.github.io/pipx/#install-pipx)
  2. Use `pipx` to install AlphaMind to avoid dependency conflicts with other python packages.
     ```shell
     pipx install alphamind
     ```
  3. Now start `alphamind` using the standard steps described earlier

#### Install fails while building Tokenizer dependency
- **Details**: `pip install alphamind` fails while building the `tokenizers` dependency. Complains about Rust.
- **Fix**: Install Rust to build the tokenizers package. For example on Mac run:
    ...
    source ~/.cargo/env
    ```
- **Refer**: [Issue with Fix](https://github.com/alphamind-ai/alphamind/issues/82#issuecomment-1241890946) for more details

#### AlphaMind in Docker errors out with \"Killed\" in error message
